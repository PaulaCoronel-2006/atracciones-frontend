...
<template>
  <div class="flex flex-col gap-10">
    ...
    <section class="relative rounded-3xl overflow-hidden py-16 md:py-24 px-6 text-center border border-dark-700 bg-gradient-to-br from-dark-800 to-dark-900/60 shadow-xl transition-all duration-300">
      <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-brand-cyan/5 via-transparent to-transparent pointer-events-none"></div>
      
      <div class="relative z-10 max-w-3xl mx-auto flex flex-col gap-6">
        <h1 class="text-4xl md:text-6xl font-black tracking-tight text-text-primary font-sans leading-tight">
        ...
        <p class="text-text-secondary text-base md:text-lg max-w-xl mx-auto">
          Reserva visitas guiadas, cruceros privados y experiencias premium en los rincones más espectaculares de la región.
        </p>

        <!-- Buscador Global -->
        <div class="mt-4 flex max-w-xl mx-auto w-full rounded-2xl glass-panel p-2 border border-dark-700 shadow-xl transition-all duration-300">
          <div class="flex items-center pl-3 text-text-secondary">
          ...
          <input 
            ...
            class="w-full bg-transparent border-none text-text-primary placeholder-text-secondary/50 pl-3 py-2 text-sm focus:outline-none"
          />
        </div>
      </div>
    </section>
    ...
    <section class="flex flex-col gap-4 text-left">
      <h2 class="text-xs font-bold text-text-secondary uppercase tracking-wider">Filtrar por Categoría</h2>
        ...
        <button 
          ...
          :class="[
            selectedCategoryId === cat.id 
              ? 'border-brand-cyan ring-2 ring-brand-cyan/30 shadow-lg shadow-brand-cyan/10 scale-102' 
              : 'border-dark-700 hover:border-brand-cyan/40 hover:scale-102'
          ]"
        >
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-dark-900/90 via-dark-800/40 to-transparent"></div>
    ...
    <section class="flex flex-col gap-6 text-left">
      <div class="flex items-center justify-between border-b border-dark-700 pb-3">
        <h2 class="text-sm font-bold text-text-primary uppercase tracking-wider flex items-center gap-2">
          ...
          <span class="text-xs px-2 py-0.5 rounded-full bg-dark-800 border border-dark-700 text-text-secondary font-normal">
      ...
      <div v-else-if="filteredAttractions.length === 0" class="glass-card rounded-2xl p-12 text-center border border-dark-700">
        ...
        <button @click="searchQuery = ''; selectedCategoryId = ''" class="mt-4 px-4 py-2 rounded-xl text-xs font-bold bg-brand-cyan text-white dark:text-dark-900 hover:bg-brand-cyan/80 transition-all cursor-pointer">
          Limpiar Filtros
        </button>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div 
          ...
          :key="attr.id"
          class="rounded-2xl overflow-hidden glass-panel glass-panel-hover flex flex-col group border border-dark-700"
        >
          ...
          <div class="relative h-48 overflow-hidden">
            ...
            <!-- Badge de Puntuación -->
            <div class="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-dark-900/80 backdrop-blur-md border border-dark-700 flex items-center gap-1 text-xs font-bold text-yellow-500">
              <span>★</span>
              <span>{{ attr.rating.toFixed(2) }}</span>
            </div>

            <!-- Ubicación en Badge Superior -->
            <div class="absolute bottom-3 left-3 px-2.5 py-1 rounded-lg bg-dark-900/80 backdrop-blur-md border border-dark-700 text-[10px] font-bold text-brand-cyan uppercase tracking-wider">
          ...
          <div class="p-5 flex-grow flex flex-col gap-3">
            ...
            <!-- Tags -->
            <div class="flex flex-wrap gap-1.5" v-if="attr.tags?.length > 0">
              <span 
                v-for="tagId in attr.tags" 
                :key="tagId"
                class="text-[9px] font-bold px-2 py-0.5 rounded bg-dark-800 border border-dark-700 text-text-secondary"
              >
            ...
            <!-- Título -->
            <h3 class="font-extrabold text-base md:text-lg text-text-primary group-hover:text-brand-cyan transition-colors leading-snug">
              {{ attr.name }}
            </h3>

            <!-- Descripción Corta -->
            <p class="text-text-secondary text-xs line-clamp-3 leading-relaxed">
              {{ attr.description }}
            </p>
            ...
            <div class="flex items-center justify-between mt-auto">
              <div class="flex flex-col">
                <span class="text-[9px] text-text-secondary uppercase tracking-wider">Desde</span>
                <span class="text-base font-black text-brand-emerald">
                  ${{ attr.price_base.toFixed(2) }} <span class="text-[10px] font-normal text-text-secondary">/ pers</span>
                </span>
              </div>
              ...
              <router-link 
                ...
                class="px-3.5 py-2 rounded-xl text-xs font-bold bg-brand-cyan text-white dark:text-dark-900 hover:bg-brand-cyan/80 hover:shadow-lg hover:shadow-brand-cyan/20 transition-all flex items-center gap-1 cursor-pointer"
              >
                Ver Detalle
